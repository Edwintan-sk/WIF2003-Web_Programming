...
const KPICard = ({ data }) => (
  <Row className="align-items-center p-4 kpi-row mx-0">
    
    {/* Info Section */}
    <Col xs={12} md={4} className="mb-3 mb-md-0 px-0 pe-md-3">
      <div className="d-flex gap-2 mb-2">
        <span 
          className="badge text-xs kpi-tag font-monospace" 
          style={{ backgroundColor: data.tag1Bg, color: data.tag1Text}}>
          {data.tag1}
        </span>
        <span className="badge text-uppercase text-xs text-secondary kpi-tag kpi-tag-outline font-monospace" >
          {data.tag2}
        </span>
      </div>
    ...
    <Col xs={12} md={4} className="px-0 px-md-4 mb-3 mb-md-0">
      <div className="d-flex align-items-baseline gap-2 mb-2">
        ...
        <span className="text-sm text-secondary">{data.subProgress}</span>
      </div>
        ...
        <div className="d-flex align-items-center w-75 pt-1">
          {data.steps.map((state, i) => (
             <React.Fragment key={i}>
                ...
                {/* Icons */}
                {state === 'current' && <Circle className="d-flex timeline-circle-current" color="#597495" size={14} />}
                {state === 'pending' && <CircleFill className="d-flex" color="#e8e4d9" size={14} />}

                {/* Timeline */}
                {i < data.steps.length - 1 && (
                    <div className="flex-grow-1 timeline-line" style={{ 
    ...
    <Col xs={6} md={2} className="d-flex flex-column align-items-start px-0">
      <span className="mb-1 text-mini text-secondary">Due</span>
      <span className="fw-bold mb-2 text-sm">{data.due}</span>
    </Col>

    {/* Action Section */}
    <Col xs={6} md={2} className="d-flex align-items-center justify-content-end gap-3 px-0">
      <div className="d-flex align-items-center gap-2 px-3 py-1 rounded-pill fw-bold text-mini" 
      ...
      <button className="btn rounded-circle btn-action-arrow flex-shrink-0 d-flex align-items-center justify-content-center p-0 border-0">
            ...
            <main>
              ...
              <div className="d-flex justify-content-between align-items-end mb-4">
                ...
                <div className="d-flex gap-3">
                    <div className="rounded p-1 shadow-sm d-flex fw-medium text-sm bg-white">
                      {/* Clickable Filters */}
                      {['All types', 'Target', 'Project'].map(type => (
                      <button key={type} onClick={() => setActiveType(type)} 
                        className={`btn border-0 rounded px-3 py-1 shadow-none text-sm tab-btn ${activeType === type ? 'active-tab' : 'text-secondary'}`}>
                          {type}
                      </button>
                    ))}
                  </div>
                ...
                {/* Filter Pills */}
                <div className="d-flex gap-3 mb-4 overflow-auto pb-2 hide-scrollbar">
                {filterPills.map((filter, idx) => {
                  const isActive = activePill === filter.label;
                  return (
                    <button key={idx} onClick={() => setActivePill(filter.label)}
                        className={`btn rounded-pill d-flex align-items-center gap-2 px-3 py-1 fw-medium text-nowrap filter-btn shadow-sm text-sm ${isActive ? 'active' : ''}`}>
                          {filter.label}
                          <span className="badge rounded-pill px-2 py-1 text-mini">
                            {filter.count}
                        </span>
                    </button>
                  );
